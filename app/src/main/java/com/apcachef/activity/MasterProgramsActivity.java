...
public class MasterProgramsActivity extends BaseActivity implements OnListItemClickListener {
    ImageView img_back;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv_title.setText("MASTER CHEF SERIES");
...
//
     ...
     /*   courseDetailRequest.setId("18");
        ...
        call.enqueue(new Callback<CourseDetailResponse>() {
            ...
            public void onResponse(Call<CourseDetailResponse> call, Response<CourseDetailResponse> response) {
               ...
               desc = response.body().getData().getRow().getChef_about();
    ...
    public void onBackPressed() {
       ...
       /*
        extends BaseActivity implements OnListItemClickListener {
    ImageView img_back;
    SharedPrefsHelper sharedPrefsHelper;
    TextView tv_title, tv_header, tv_video_title, tv_description;
    RecyclerView recyclerPrograms;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_master_programs);
        ...
        topics = bundle.getParcelableArrayList("topicsList");
        id = getIntent().getStringExtra("id");
        name = getIntent().getStringExtra("name");
        title = getIntent().getStringExtra("title");
        video_url = getIntent().getStringExtra("video_url");
        ...
        tv_video_title.setText(name);
        tv_description.setText(desc);
        tv_header.setText(title);

        recyclerPrograms = findViewById(R.id.recyclerPrograms);
        topicsAdapter = new TopicsAdapter(getApplicationContext(), topics, this);
        recyclerPrograms.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
        recyclerPrograms.setAdapter(topicsAdapter);
        recyclerPrograms.setNestedScrollingEnabled(false);
    ...
    public void onItemClick(int position, String type) {
        if (is_subscribed == 1) {
            List<RecipeItem> recipeList = AppConstants.topics.get(position).getRecipes();
            Intent intent = new Intent(getApplicationContext(), RecipeListActivity.class);
            Bundle bundle = new Bundle();
            bundle.putString("id", topics.get(position).getId());
            bundle.putString("title", topics.get(position).getTitle());
